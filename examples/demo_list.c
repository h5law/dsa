...
int main(void)
{
    ...
    const unsigned char *data  = calloc(1, data_len);
    const unsigned char *data2 = calloc(1, data2_len);
    const unsigned char *data3 = calloc(1, data3_len);
    const unsigned char *data4 = calloc(1, data4_len);

    data                       = memcpy(( void * )data, pre_data, data_len);
    data2                      = memcpy(( void * )data2, pre_data2, data2_len);
    data3                      = memcpy(( void * )data3, pre_data3, data3_len);
    data4                      = memcpy(( void * )data4, pre_data4, data4_len);
    ...
    found = ll_peek(&list);
    assert(memcmp(( const uint8_t * )found->data, ( const uint8_t * )data,
                  data_len) == 0);
    assert(list.size == 1);
    ...
    printf("Testing ll_peek\n");
    found = ll_peek(&list);
    assert(memcmp(( const uint8_t * )found->data, ( const uint8_t * )data2,
                  data2_len) == 0);
    assert(list.size == 2);
    ...
    printf("Testing ll_peek\n");
    found = ll_peek(&list);
    assert(memcmp(( const uint8_t * )found->data, ( const uint8_t * )data4,
                  data4_len) == 0);
    assert(list.size == 2);
